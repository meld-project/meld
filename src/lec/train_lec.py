...
class TrainConfig:
    model_dir: str
    split_mode: str = "cv"  # cv / holdout / dir
    ...
    def validate(self) -> None:
        if self.split_mode not in {"cv", "holdout", "dir", "time_ood"}:
            raise ValueError(f"split_mode 必须是 cv/holdout/dir/time_ood 之一，当前为 {self.split_mode}")
...
def build_parser(defaults: Dict) -> argparse.ArgumentParser:
    ...
    parser.add_argument("--split_mode", choices=["cv", "holdout", "dir", "time_ood"], default=defaults.get("split_mode", "cv"))
...
def evaluate_cv_layers(
    ...
    for li in layer_iter:
        ...
        if best_result is None or metrics["drift_score"] > best_result["drift_score"]:
            best_result = metrics
...
def evaluate_holdout_layers(
    ...
    for li in layer_iter:
        ...
        val_prob = predict_proba(pipeline, X_va[:, li, :])
        id_stats = quantile_threshold(y_va, val_prob, target_fpr=target_fpr)
        te_prob = predict_proba(pipeline, X_te[:, li, :])
        ood_stats = _evaluate_fixed_threshold(y_te, te_prob, tau=id_stats["tau"])
        try:
        ...
        kl_stats = compute_layer_kl_metrics(val_prob, y_va, te_prob, y_te)
        ...
        metrics = {
            ...
            "num_samples_val": num_samples_va,
        ...
        if bootstrap_samples > 0:
            rng_seed = seed + li * 10
            pos_val = val_prob[y_va == 1]
            neg_val = val_prob[y_va == 0]
            pos_te = te_prob[y_te == 1]
            neg_te = te_prob[y_te == 0]
            if pos_val.size > 0:
                ci = bootstrap_interval(pos_val, id_stats["tau"], bootstrap_samples, rng_seed)
                metrics["tpr_id_ci"] = [float(ci[0]), float(ci[1])]
            if neg_val.size > 0:
                ci = bootstrap_interval(neg_val, id_stats["tau"], bootstrap_samples, rng_seed + 1)
                metrics["fpr_id_ci"] = [float(ci[0]), float(ci[1])]
            if pos_te.size > 0:
                ci = bootstrap_interval(pos_te, id_stats["tau"], bootstrap_samples, rng_seed + 2)
                metrics["tpr_ood_ci"] = [float(ci[0]), float(ci[1])]
            if neg_te.size > 0:
                ci = bootstrap_interval(neg_te, id_stats["tau"], bootstrap_samples, rng_seed + 3)
...
def run_experiment(cfg: TrainConfig) -> Dict:
    ...
    if cfg.split_mode == "dir":
        ...
        def load_texts_from_paths(paths):
            ...
            for path in paths:
                try:
                    ...
                    texts.append("")
                    paths_list.append(str(path))
            return texts, paths_list
        
        train_texts, train_paths_str = load_texts_from_paths(train_paths)
        val_texts, val_paths_str = load_texts_from_paths(val_paths)
        test_texts, test_paths_str = load_texts_from_paths(test_paths)
        ...
        summary = evaluate_holdout_layers(
            X_tr=datasets["train"][0],
            y_tr=datasets["train"][1],
            X_va=datasets["val"][0],
            y_va=datasets["val"][1],
            X_te=datasets["test"][0],
        ...
        if cfg.split_mode == "cv":
            ...
            if "all_layers" in summary:
                ...
                # 记录最佳层的详细可视化
                best_layer_idx = summary["best"].get("layer_index", 1) - 1
                if best_layer_idx >= 0 and best_layer_idx < X_layers.shape[1]:
                    # 重新计算最佳层的预测概率用于可视化
                    X_best = X_layers[:, best_layer_idx, :]
            ...
            summary = evaluate_holdout_layers(
                X_tr=X_layers[train_idx],
                y_tr=y[train_idx],
                X_va=X_layers[val_idx],
                y_va=y[val_idx],
                X_te=X_layers[test_idx],
                y_te=y[test_idx],
                clf_name=cfg.clf,
                target_fpr=cfg.target_fpr,
                lambda_penalty=cfg.lambda_penalty,
